...
import {
  ...
  deleteDoc, writeBatch, getDoc, query, where, orderBy
...
async function fbGetUsuarios(){
  ...
  } catch(e){ console.error("Error leyendo usuarios:",e); return null; }
}
...
async function fbGetProductos(){
  ...
  } catch(e){ console.error("Error leyendo productos:",e); return null; }
}
...
async function fbGetCotizaciones(usuario, isAdmin){
  try {
    ...
    // Admin ve todo; cliente y vendedor solo las suyas
    if(!isAdmin) data=data.filter(c=>c.usuario===usuario);
    return data.sort((a,b)=>new Date(b.fecha)-new Date(a.fecha));
  } catch(e){ console.error("Error leyendo cotizaciones:",e); return null; }
}
...
function canDo(session, accion){
  ...
  if(rol==="admin"){
    return ["productos","clientes","subir_csv","crear_cliente","editar_cliente","toggle_estatus","eliminar_cliente"].includes(accion);
  }
  return false;
}
function isAdminRole(session){ return session?.rol==="admin"||session?.rol==="superadmin"; }
function isVendedor(session){ return session?.lista==="VENDEDOR"||isAdminRole(session); }
...
function Badge({val}){
  const map={
    ...
    superadmin:{bg:"#fef3c7",c:"#d97706"},admin:{bg:"#dbeafe",c:"#2563eb"},
    client:{bg:"#f3f4f6",c:"#6b7280"},
    activo:{bg:"#dcfce7",c:"#16a34a"},inactivo:{bg:"#fee2e2",c:"#dc2626"}
  };
...
function Logo({h=36}){
  return <img src="https://raw.githubusercontent.com/nicobuenrostro/portal-tapatia/main/logo.png"
    alt="Grupo Tapatía" style={{height:h,objectFit:"contain",maxWidth:280}}/>;
}
...
// ── PDF ───────────────────────────────────────────────────────
async function generarPDF({folio,session,items,nota,vigencia,descuento}){
  const doc2=new jsPDF({orientation:"portrait",unit:"mm",format:"a4"});
  const W=210,M=15;
  doc2.setFillColor(255,107,6); doc2.rect(0,0,W,38,"F");
  doc2.setTextColor(255,255,255); doc2.setFontSize(22); doc2.setFont("helvetica","bold");
  ...
  doc2.setFillColor(230,90,0); doc2.rect(140,6,55,26,"F");
  doc2.setTextColor(255,255,255); doc2.setFontSize(14); doc2.setFont("helvetica","bold");
  doc2.text("COTIZACIÓN",167.5,17,{align:"center"});
  doc2.setFontSize(11); doc2.text(folio,167.5,26,{align:"center"});
  const fecha=new Date().toLocaleDateString("es-MX",{day:"2-digit",month:"long",year:"numeric"});
  doc2.setTextColor(60,60,60); doc2.setFontSize(9); doc2.setFont("helvetica","normal");
  let y=46;
  doc2.setFillColor(245,245,245); doc2.rect(M,y-5,W-M*2,22,"F");
  doc2.setFont("helvetica","bold"); doc2.text("DATOS DE COTIZACIÓN",M+2,y); y+=5;
  doc2.setFont("helvetica","normal"); doc2.text("Folio:",M+2,y); doc2.setFont("helvetica","bold"); doc2.text(folio,M+22,y);
  doc2.setFont("helvetica","normal"); doc2.text("Fecha:",90,y); doc2.setFont("helvetica","bold"); doc2.text(fecha,108,y); y+=5;
  doc2.setFont("helvetica","normal"); doc2.text("Elaboró:",M+2,y); doc2.setFont("helvetica","bold"); doc2.text(session.nombre,M+22,y);
  doc2.setFont("helvetica","normal"); doc2.text("Vigencia:",90,y); doc2.setFont("helvetica","bold"); doc2.text(vigencia||"15 días naturales",108,y); y+=5;
  doc2.setFont("helvetica","normal"); doc2.text("Cliente:",M+2,y); doc2.setFont("helvetica","bold"); doc2.text(session.empresa||session.nombre,M+22,y);
  y+=10;
  const rows=items.map((it,i)=>[i+1,it.codigo,it.descripcion,it.cantidad,
  ...
  doc2.autoTable({startY:y,head:[["#","CÓDIGO","DESCRIPCIÓN","CANT.","P. UNIT.","IMPORTE"]],body:rows,
    margin:{left:M,right:M},
    headStyles:{fillColor:[255,107,6],textColor:255,fontStyle:"bold",fontSize:8,halign:"center"},
    bodyStyles:{fontSize:8,textColor:[40,40,40]},
    columnStyles:{0:{halign:"center",cellWidth:8},1:{cellWidth:30},2:{cellWidth:80},3:{halign:"center",cellWidth:14},4:{halign:"right",cellWidth:25},5:{halign:"right",cellWidth:25}},
    alternateRowStyles:{fillColor:[250,250,250]},tableLineColor:[220,220,220],tableLineWidth:0.1});
  const subtotal=items.reduce((s,it)=>s+it.precio*it.cantidad,0);
  const descMonto=descuento>0?subtotal*(descuento/100):0;
  const total=subtotal-descMonto;
  const finalY=doc2.lastAutoTable.finalY+6;
  doc2.setFillColor(245,245,245); doc2.rect(120,finalY-4,75,descuento>0?28:20,"F");
  doc2.setFont("helvetica","normal"); doc2.setFontSize(9); doc2.setTextColor(80,80,80);
  doc2.text("Subtotal:",122,finalY+2);
  if(descuento>0){ doc2.text(`Descuento (${descuento}%):`,122,finalY+8); doc2.text("IVA:",122,finalY+14); }
  else { doc2.text("IVA:",122,finalY+8); }
  ...
  if(descuento>0){
    ...
    doc2.text("-"+new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN"}).format(descMonto),193,finalY+8,{align:"right"});
    doc2.setTextColor(40,40,40);
    doc2.setFontSize(8); doc2.setFont("helvetica","normal"); doc2.setTextColor(100,100,100);
    doc2.text("(Según tipo de producto — consultar factura)",193,finalY+14,{align:"right"});
  } else {
    doc2.setFontSize(8); doc2.setFont("helvetica","normal"); doc2.setTextColor(100,100,100);
    doc2.text("(Según tipo de producto — consultar factura)",193,finalY+8,{align:"right"});
  }
  const lineY=finalY+(descuento>0?16:10);
  doc2.setDrawColor(255,107,6); doc2.setLineWidth(0.5); doc2.line(120,lineY,195,lineY);
  doc2.setFillColor(255,107,6); doc2.rect(120,lineY+1,75,9,"F");
  doc2.setTextColor(255,255,255); doc2.setFont("helvetica","bold"); doc2.setFontSize(11);
  doc2.text("TOTAL:",122,lineY+7.5);
  doc2.text(new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN"}).format(total),193,lineY+7.5,{align:"right"});
  if(nota){
    const ny=lineY+18;
    doc2.setTextColor(60,60,60); doc2.setFont("helvetica","bold"); doc2.setFontSize(8);
    doc2.text("OBSERVACIONES:",M,ny);
    doc2.setFont("helvetica","normal");
    doc2.text(doc2.splitTextToSize(nota,W-M*2-5),M,ny+5);
  }
  const py=282;
  doc2.setFillColor(255,107,6); doc2.rect(0,py,W,15,"F");
...
// ── CartPanel ─────────────────────────────────────────────────
// canDiscount: solo vendedor/admin pueden aplicar descuento adicional
function CartPanel({cart,setCart,session,db,onClose,mob}){
  const vend=isVendedor(session);
  // ↓ NUEVO: clientes (rol=client y lista≠VENDEDOR) NO pueden aplicar descuento
  const canDiscount=vend;
  ...
  async function getNextFolio(){
    ...
    return`COT-${session.usuario.substring(0,3).toUpperCase()}-${String(next).padStart(4,"0")}`;
  }
  ...
  async function generarCotizacion(){
    ...
    try{
      ...
      const descReal=canDiscount?descuento:0;
      await generarPDF({folio,session,items:cart,nota,vigencia,descuento:descReal});
      await setDoc(doc(db,"cotizaciones",folio),{
        ...
        subtotal,descuento:descReal,total:subtotal-(subtotal*(descReal/100)),
      ...
      <div style={{width:"100%",maxWidth:mob?480:560,background:"#fff",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 32px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{background:OR,padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div>
            ...
            <div style={{color:"rgba(255,255,255,0.85)",fontSize:12,marginTop:2}}>
              {cart.length} producto{cart.length!==1?"s":""} · <strong>{money2(total)}</strong>
            </div>
          </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",border:"1px solid #d1d5db",borderRadius:6,overflow:"hidden"}}>
                    <button onClick={()=>updCantidad(i,it.cantidad-1)} style={{background:"#f3f4f6",border:"none",padding:"5px 10px",cursor:"pointer",fontSize:15,fontWeight:700,color:"#374151",lineHeight:1}}>−</button>
                ...
                {/* Selector de precio — solo vendedor/admin */}
                {vend&&(
        ...
        {cart.length>0&&(
          <div style={{borderTop:"2px solid #e5e7eb",padding:"14px 16px",background:"#fff",flexShrink:0}}>
            <div style={{display:"grid",gridTemplateColumns:canDiscount?"1fr 1fr 1fr":"1fr 1fr",gap:"0 10px",marginBottom:10}}>
              <div>
              ...
              {/* Descuento adicional — SOLO vendedor/admin */}
              {canDiscount&&(
                <div>
                  <div style={{color:GRL,fontSize:10,letterSpacing:2,marginBottom:4}}>DESCUENTO %</div>
                  <input type="number" min="0" max="50" value={descuento} onChange={e=>setDescuento(Math.min(50,Math.max(0,Number(e.target.value)||0)))}
              ...
              <div>
                ...
                <input value={nota} onChange={e=>setNota(e.target.value)} placeholder="Notas adicionales..."
                  style={{width:"100%",padding:"7px 9px",border:"1px solid #d1d5db",borderRadius:6,fontSize:12,outline:"none",boxSizing:"border-box"}}/>
              </div>
            </div>
            {/* Resumen */}
            <div style={{background:"#f9f9f9",border:"1px solid #e5e7eb",borderRadius:8,padding:"10px 14px",marginBottom:12}}>
              ...
              {canDiscount&&descuento>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:12,color:GRL}}>Subtotal:</span>
                <span style={{fontSize:12,color:GRL}}>{money2(subtotal)}</span>
              </div>}
              {canDiscount&&descuento>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:12,color:"#dc2626"}}>Descuento ({descuento}%):</span>
                <span style={{fontSize:12,color:"#dc2626",fontWeight:700}}>-{money2(descMonto)}</span>
              </div>}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:"1px solid #e5e7eb",paddingTop:8,marginTop:4}}>
                <span style={{fontSize:14,fontWeight:700,color:"#1a1a1a"}}>TOTAL ANTES DE IVA:</span>
                <span style={{fontSize:20,fontWeight:800,color:OR}}>{money2(total)}</span>
              </div>
            </div>
...
function HistorialCotizaciones({session,db,mob}){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(data!==null) setCotizaciones(data);
      setLoading(false);
    })();
  },[]);

  async function reimprimirPDF(cot){
    await generarPDF({
      folio:cot.folio,session:{nombre:cot.nombre,empresa:cot.empresa},
      items:cot.items,nota:cot.nota,vigencia:cot.vigencia,descuento:cot.descuento||0
    });
  }
  ...
  if(cotizaciones.length===0) return(
    <div style={{textAlign:"center",padding:"50px 20px",color:GRL}}>
      ...
      <div style={{fontSize:12,marginTop:6}}>Las cotizaciones generadas aparecerán aquí</div>
    </div>
  );

  return(
    <div>
      <div style={{marginBottom:12,color:GRL,fontSize:11}}>{cotizaciones.length} cotización{cotizaciones.length!==1?"es":""}</div>
      {cotizaciones.map(cot=>{
        const open=expanded===cot.id||expanded===cot.folio;
        return(
          <div key={cot.folio} style={{background:CD,border:"1px solid "+BD,borderRadius:6,marginBottom:8,boxShadow:"0 1px 4px rgba(0,0,0,0.04)",overflow:"hidden"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",cursor:"pointer"}}
              onClick={()=>setExpanded(open?null:(cot.id||cot.folio))}>
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:12,marginTop:3,flexWrap:"wrap"}}>
                  <span style={{fontSize:11,color:GRL}}>{new Date(cot.fecha).toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}</span>
                  <span style={{fontSize:11,color:"#1a1a1a",fontWeight:600}}>{money2(cot.total||cot.subtotal)}</span>
                  <span style={{fontSize:11,color:GRL}}>{(cot.items||[]).length} prod · {(cot.items||[]).reduce((s,it)=>s+it.cantidad,0)} pzas</span>
                </div>
              </div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <button onClick={e=>{e.stopPropagation();reimprimirPDF(cot);}}
                  style={{background:OR,color:"#fff",border:"none",padding:"6px 12px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:700}}>
                  📄 PDF
                </button>
                <span style={{color:GRL,fontSize:16,transform:open?"rotate(180deg)":"rotate(0)",transition:"transform 0.2s"}}>▾</span>
              </div>
            </div>
            {open&&(
              <div style={{borderTop:"1px solid "+BD,padding:"12px 16px",background:"#fafafa"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                  <thead><tr style={{background:"#f0f0f0"}}>
                    {["CÓDIGO","DESCRIPCIÓN","CANT.","P. UNIT.","IMPORTE"].map(h=><th key={h} style={{padding:"6px 10px",textAlign:h==="CANT."||h==="P. UNIT."||h==="IMPORTE"?"right":"left",color:OR,fontWeight:700,fontSize:10}}>{h}</th>)}
                  </tr></thead>
                  <tbody>{(cot.items||[]).map((it,i)=><tr key={i} style={{borderTop:"1px solid #f0f0f0"}}>
                    <td style={{padding:"5px 10px",fontFamily:"monospace",color:GRL}}>{it.codigo}</td>
                    ...
                    <td style={{padding:"5px 10px",textAlign:"right",fontWeight:600,color:OR}}>{money2(it.precio*it.cantidad)}</td>
                  </tr>)}</tbody>
                </table>
                <div style={{display:"flex",justifyContent:"flex-end",gap:20,marginTop:10,paddingTop:8,borderTop:"1px solid #e5e7eb"}}>
                  <span style={{fontSize:12,color:GRL}}>Subtotal: <strong>{money2(cot.subtotal)}</strong></span>
                  {(cot.descuento||0)>0&&<span style={{fontSize:12,color:"#dc2626"}}>Descuento ({cot.descuento}%): <strong>-{money2(cot.subtotal*(cot.descuento/100))}</strong></span>}
                  <span style={{fontSize:13,fontWeight:700,color:OR}}>Total: {money2(cot.total||cot.subtotal)}</span>
                </div>
                {cot.nota&&<div style={{marginTop:8,fontSize:11,color:GRL}}><strong>Nota:</strong> {cot.nota}</div>}
                {cot.vigencia&&<div style={{fontSize:11,color:GRL}}><strong>Vigencia:</strong> {cot.vigencia}</div>}
              </div>
            )}
          </div>
        );
...
function PassCell({uid,db,hashPassword}){
  ...
  return(
    <div style={{display:"flex",gap:4,alignItems:"center"}}>
      ...
      <button onClick={()=>setShow(s=>!s)} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,padding:"2px 4px",color:GRL}} title={show?"Ocultar":"Mostrar"}>{show?"🙈":"👁"}</button>
      <button onClick={()=>{setEditing(true);setShow(false);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,padding:"2px 4px",color:OR}} title="Cambiar contraseña">✏️</button>
...
function ChangePassword({session,db,hashPassword,checkPassword}){
  ...
  async function cambiar(){
    ...
    try {
      ...
      if(!ok){setMsg("❌ Contraseña actual incorrecta.");setLoading(false);return;}
      ...
      setMsg("✅ Contraseña cambiada.");setActual("");setNueva("");setConfirmar("");
    }catch(e){setMsg("❌ Error: "+e.message);}
    setLoading(false);
  }
  return <div>
    <div style={{display:"grid",gap:10}}>
      {[["CONTRASEÑA ACTUAL",actual,setActual],["CONTRASEÑA NUEVA",nueva,setNueva],["CONFIRMAR",confirmar,setConfirmar]].map(([lbl,val,set])=>(
        <div key={lbl}>
...
function CreateAdmin({session,db,hashPassword}){
  ...
  async function crear(){
    ...
    try {
      const snap=await getDocs(collection(db,"usuarios"));
      if(snap.docs.find(d=>d.data().usuario===form.usuario.trim())){setMsg("❌ Usuario ya existe.");setLoading(false);return;}
      ...
      await setDoc(doc(db,"usuarios","admin_"+Date.now()),{
        ...
        creado_por:session.nombre,creado_en:new Date().toISOString(),actualizado:new Date().toISOString()
      });
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
      {[["NOMBRE","nombre"],["USUARIO","usuario"],["CONTRASEÑA","password"],["CONFIRMAR","confirmar"]].map(([lbl,k])=>(
        <div key={k} style={{marginBottom:12}}>
          <div style={{color:GRL,fontSize:10,letterSpacing:2,marginBottom:4}}>{lbl}</div>
          <input type={k.includes("password")||k==="confirmar"?"password":"text"} value={form[k]} onChange={e=>upd(k,e.target.value)}
            style={{width:"100%",padding:"9px 11px",background:"#f7f7f7",border:"1px solid "+BD,color:"#1a1a1a",fontSize:13,borderRadius:4,boxSizing:"border-box",outline:"none"}}/>
        </div>
      ))}
    </div>
...
export default function App(){
  ...
  function addToCart(p){
    const lista=session?.lista;
    const vend=isVendedor(session);
    // Precio inicial según tipo de usuario
    const tipoPrecio=vend?"publico":lista==="DISTRIBUIDOR"?"distribuidor":lista==="ASOCIADO"?"asociado":"publico";
    const precioInicial=tipoPrecio==="publico"?Number(p.publico)||0:tipoPrecio==="distribuidor"?Number(p.distribuidor)||0:Number(p.asociado)||0;
    setCart(prev=>{
      const existe=prev.findIndex(it=>it.codigo===p.codigo);
      if(existe>=0) return prev.map((it,i)=>i===existe?{...it,cantidad:it.cantidad+1}:it);
      return [...prev,{
        codigo:p.codigo,descripcion:p.descripcion,
        precio:precioInicial,tipoPrecio,cantidad:1,
  ...
  function doLogout(){
    setSession(null);setView("login");setSearch("");setDs("");setPage(0);
    setProducts([]);setUsers([]);localStorage.removeItem("gt_session");
  }
    ...
    reader.onload=async ev=>{
      try {
        ...
        if(oldSnap.docs.length>0){
          ...
          const bk=writeBatch(db);
          oldSnap.docs.forEach(d=>bk.set(doc(db,`respaldo_${ts}`,d.id),d.data()));
          await bk.commit();
        ...
        for(let i=0;i<mapped.length;i+=chunk){
          ...
          await batch.commit();
          setMsg(`⏳ ${Math.min(i+chunk,mapped.length)} / ${mapped.length} guardados...`);
        }
        const verify=await getDocs(collection(db,"productos"));
        if(verify.size===0){setMsg("❌ ERROR: Los productos no se guardaron. Revisa Firebase.");return;}
        await setDoc(doc(db,"bitacora",`carga_${Date.now()}`),{tipo:"carga_productos",por:session.nombre,cantidad:mapped.length,fecha:new Date().toISOString()});
        await loadProducts();
        setMsg(`✅ ${mapped.length} productos guardados.`);
  ...
  async function saveClient(form){
    setSaving(true);
    try {
      if(!form.nombre?.trim()||!form.usuario?.trim()){alert("Nombre y usuario son obligatorios.");setSaving(false);return;}
      if(!form.id&&!form.password?.trim()){alert("La contraseña es obligatoria para usuarios nuevos.");setSaving(false);return;}
      if(!form.id&&users.find(u=>u.usuario.trim()===form.usuario.trim())){alert("Ya existe ese usuario.");setSaving(false);return;}
      const id=form.id||"u_"+Date.now();
      const data={nombre:form.nombre.trim(),empresa:(form.empresa||"").trim(),usuario:form.usuario.trim(),
        lista:form.lista,estatus:form.estatus,rol:"client",actualizado:new Date().toISOString()};
      ...
      await setDoc(doc(db,"bitacora",`u_${Date.now()}`),{tipo:form.id?"edicion_usuario":"nuevo_usuario",usuario:form.usuario.trim(),por:session.nombre,fecha:new Date().toISOString()});
  ...
  async function toggleEstatus(id,est){
    try {
      await setDoc(doc(db,"usuarios",id),{estatus:est==="activo"?"inactivo":"activo",actualizado:new Date().toISOString()},{merge:true});
  ...
  async function deleteClient(id,nombre,rol){
    ...
    try {
      ...
      await setDoc(doc(db,"bitacora",`del_${Date.now()}`),{tipo:"eliminacion_usuario",usuario_id:id,usuario_nombre:nombre,por:session.nombre,fecha:new Date().toISOString()});
  ...
  const filtered=useMemo(()=>{
    ...
    return results.sort((a,b)=>{
      ...
      if(ta===0&&tb===0)return 0;if(ta===0)return 1;if(tb===0)return -1;
      return tb-ta;
    });
  },[products,ds]);
  ...
  return(
    <div style={{minHeight:"100vh",background:DK,fontFamily:"Arial,sans-serif",color:"#1a1a1a"}}>
      ...
      {/* Sub-tabs cliente: Catálogo y Mis Cotizaciones */}
      <div style={{background:CD,display:"flex",borderBottom:"1px solid "+BD,padding:mob?"0 8px":"0 24px",overflowX:"auto",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
              ...
              <div>{filtered.slice(page*PS,(page+1)*PS).map((p,i)=>{
                ...
                return <div key={i} style={{background:CD,border:"1px solid "+BD,borderRadius:6,padding:12,marginBottom:8,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                    {ALMS.map((a,idx)=>{const v=Number(p[a])||0;return<div key={a} style={{background:v>0?"#f0fdf4":"#f9f9f9",border:"1px solid "+(v>0?"#bbf7d0":BD),borderRadius:3,padding:"2px 7px",textAlign:"center"}}>
                      <div style={{fontSize:9,color:GRL}}>{ALMS_L[idx]}</div>
                      <div style={{fontSize:11,fontWeight:700,color:v>0?nColor(v):"#ccc"}}>{v>=30?"+30":v}</div>
                    </div>;})}
                  </div>
                  <button onClick={()=>addToCart(p)} style={{width:"100%",padding:"8px",background:OR,color:"#fff",border:"none",borderRadius:4,cursor:"pointer",fontWeight:700,fontSize:12,letterSpacing:1}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  ...
                  <tbody>{filtered.slice(page*PS,(page+1)*PS).map((p,i)=>{
                    ...
                    return <tr key={i} style={{borderTop:"1px solid "+BD,background:i%2===0?CD:"#fafafa"}}>
                      ...
                      {ALMS.map(a=>{const v=Number(p[a])||0;return<td key={a} style={{padding:"7px 6px",textAlign:"right",fontSize:11,color:v>0?nColor(v):"#ccc",fontWeight:v>0?600:400}}>{v>=30?"+30":v}</td>;})}
